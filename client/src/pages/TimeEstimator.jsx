        ...
        <div className="card max-w-2xl mx-auto">
            ...
            <form onSubmit={calculateTime} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-sm font-medium mb-1 text-text-muted">Current Weight (kg)</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium mb-1 text-text-muted">Goal Weight (kg)</label>
                        <input type="number" value={goalWeight} onChange={(e) => setGoalWeight(e.target.value)} className="input-field" placeholder="70" required />
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-medium mb-1 text-text-muted">Daily Calorie Deficit/Surplus</label>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        {[250, 500, 750, 1000].map(val => (
                            <button
                                ...
                                className={`py-2 px-3 rounded-lg border transition-all ${parseInt(dailyDeficit) === val ? 'bg-primary text-darker border-primary font-bold' : 'bg-darker/50 border-text/10 text-text-muted hover:border-text/30 hover:text-text'}`}
                            >
                                {val} kcal
                            </button>
                        ))}
                    ...
                    <p className="text-xs text-text-muted mt-2">500 kcal deficit ≈ 0.5kg weight loss per week.</p>
                </div>
            ...
            {estimation && (
                <div className="mt-8 p-6 bg-card rounded-xl border border-text/10 text-center shadow-lg">
                    <p className="text-text-muted mb-2">To reach your goal, it will take approximately:</p>
                    <h3 className="text-4xl font-bold text-text mb-1">{estimation.days} Days</h3>
                    <p className="text-primary font-medium mb-6">({estimation.weeks} weeks)</p>

                    <div className="border-t border-text/10 pt-4">
                        <p className="text-sm text-text-muted">Estimated Achievement Date:</p>
                        <p className="text-lg font-semibold text-text">{estimation.date}</p>
                    </div>
                </div>
            )}
        </div>
